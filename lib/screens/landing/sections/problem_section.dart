  ...
  Widget build(BuildContext context) {
    ...
    final stats = <_PainStat>[
      _PainStat(
          metric: '64 h',
          detail:
              'wasted per clinician per quarter on documentation alone.'),
      _PainStat(
          metric: '25%',
          detail:
              'no-show rate — appointment reminders are usually manual.'),
      _PainStat(
          metric: r'$11 k',
          detail:
...
class _StatTile extends StatelessWidget {
  const _StatTile(
      {required this.stat, required this.theme, required this.cs});
