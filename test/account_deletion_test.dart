...
void main() {
  ...
  group('AccountDeletionRequest', () {
    ...
    test('statusAt is pendingGrace immediately after request', () {
      expect(build().statusAt(t0.add(const Duration(days: 1))),
    ...
    test('isInGraceWindowAt is true inside the window and false outside',
      ...
      expect(req.isInGraceWindowAt(t0.add(const Duration(days: 40))),
          isFalse);
    });

    test('isReadyToPurgeAt only flips after the grace window ends', () {
      final req = build();
      expect(req.isReadyToPurgeAt(t0.add(const Duration(days: 29))),
          isFalse);
      expect(req.isReadyToPurgeAt(t0.add(const Duration(days: 31))),
          isTrue);
    });

    test('a cancelled request never becomes ready to purge', () {
      final req =
          build(cancelledAt: t0.add(const Duration(days: 2)));
      expect(req.statusAt(t0.add(const Duration(days: 90))),
          DeletionStatus.cancelled);
      expect(req.isReadyToPurgeAt(t0.add(const Duration(days: 90))),
          isFalse);
    });

    test('a completed request reports the completed status', () {
      final req =
          build(completedAt: t0.add(const Duration(days: 31)));
      expect(req.statusAt(t0.add(const Duration(days: 32))),
    ...
    test('JSON round-trip preserves the lifecycle fields', () {
      ...
      expect(back.cancelledAt!.toUtc(),
          req.cancelledAt!.toUtc());
    ...
    test('copyWith adds a cancelledAt without disturbing the rest', () {
      final base = build();
      final cancelled =
          base.copyWith(cancelledAt: t0.add(const Duration(days: 2)));
