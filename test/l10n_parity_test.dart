...
void main() {
  ...
  setUpAll(() {
    ...
    others = {
      'tr': _loadArb('lib/l10n/intl_tr.arb'),
    };
  });
  ...
  group('i18n parity', () {
    ...
    test('translation files do not introduce stray keys not in English',
      ...
      for (final entry in others.entries) {
        ...
        expect(stray, isEmpty,
            reason:
                '${entry.key} has keys not in English template: $stray');
      }
    });
    ...
    test('locale tag on every file matches its filename', () {
      ...
      // "@@locale": "fr" inside.
      final enRaw = jsonDecode(File('lib/l10n/intl_en.arb').readAsStringSync())
          as Map<String, dynamic>;
      expect(enRaw['@@locale'], 'en');
      final trRaw = jsonDecode(File('lib/l10n/intl_tr.arb').readAsStringSync())
          as Map<String, dynamic>;
