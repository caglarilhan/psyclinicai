...
void main() {
  group('RagClient (post-Sprint-27 / F-003 close)', () {
    ...
    test(
      'query hits /v1/rag/query and respects topK + omits null docType',
      () async {
        Map<String, dynamic>? sent;
        final mock = MockClient((req) async {
    ...
    test(
      'feedback hits /v1/rag/feedback and returns server feedback_id',
      () async {
        final mock = MockClient((req) async {
    ...
    test(
      'missing ID token throws RagException(401) without hitting the network',
      () async {
        var called = false;
        final mock = MockClient((req) async {
          called = true;
