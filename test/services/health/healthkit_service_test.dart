...
void main() {
  ...
  group('PromValidation (Sprint 33 P3)', () {
    test('accepts a valid PHQ-9 sample', () {
      final r = PromValidation.validate(const PromScore(
        instrument: 'PHQ-9',
        score: 14,
  ...
  group('HealthKitService (no platform channel)', () {
    ...
    test('requestAuthorization returns false when channel unimplemented',
        () async {
    ...
    test('returns true when the native side answers true', () async {
      ...
      expect(
        await svc.writePromScore(const PromScore(
          instrument: 'PHQ-9',
          score: 14,
          takenAtIso: '2026-07-15T12:30:00Z',
        )),
    ...
    test('writePromScore rejects locally before the channel hop',
        () async {
      var called = false;
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(channel, (call) async {
      ...
      final ok = await svc.writePromScore(const PromScore(
        instrument: 'MADRS',
        score: 10,
        takenAtIso: '2026-07-15T12:30:00Z',
      ));
